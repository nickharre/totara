...
When producing or reviewing any browser-rendered interface (website, landing page, hero section, web component, or any HTML/CSS/JS output with visual intent), load and apply the **Web UI Excellence** skill from `steering/skills/web-ui-excellence-skill.md` (or the equivalent location in the target environment: `.kiro/skills/` for Kiro, `.claude/skills/` for Claude Code). This skill encodes Awwwards-calibre principles for typography, colour, layout, motion, and detail work. It is not optional for web UI work — it is the quality floor.
