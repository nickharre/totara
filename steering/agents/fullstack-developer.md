...
When implementing frontend UI — HTML, CSS, JS, or any browser-rendered interface — consult the **Web UI Excellence** skill from `steering/skills/web-ui-excellence-skill.md` (or the equivalent location in the target environment: `.kiro/skills/` for Kiro, `.claude/skills/` for Claude Code). This is Dieter's primary skill doc and your reference for:
