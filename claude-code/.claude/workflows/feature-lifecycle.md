...
- Delivery Lead prompts the user for any missing context
...
- Tester builds automation in parallel
...
Since Claude Code doesn't have native multi-agent routing, the user drives role switches:

1. Start: "Act as Gene (Delivery Lead). I want to build: <idea>"
2. Gene will tell you what context is needed and which role to invoke next
3. At each handoff: "Act as <Role>. Read `.claude/agents/<role>.md`. <Gene's handoff brief>"
4. Return to Gene for gate checks: "Act as Gene. <Role> has finished. Review the deliverable."
